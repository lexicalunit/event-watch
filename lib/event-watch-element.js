...
export default class EventWatchElement {
  ...
  render () {
    return (<div on={{click: this.didClick}} class={`${PREFIX} inline-block`}></div>)
  }
  ...
  generateTooltipTitle () {
    ...
    let tip = this.createElement('ul', `${PREFIX}-tooltip`)
  ...
  displayEvents () {
    ...
    for (let title in object) {
      ...
      for (let i = 0; i < events.length; i++) {
        let event = events[i]
        let widget = this.createElement('span', 'event-watch-item')
